	...
	it("keeps the pricing surfaces' key namespaces founding-free", () => {
		...
		for (const [locale, catalog] of catalogs) {
			for (const [key, value] of Object.entries(catalog)) {
				...
				if (
					typeof value === "string" &&
					/founding|pengasas|创始/i.test(value)
				) {
...
const REMOVED_LANDING_V2_KEYS = [
	...
	"nav_features",
	"nav_how",
	"pricing_see_full_breakdown",
	"pricing_feat_radius",
	// The hero trust line's render site (TrustLine) went with the old hero.
	"hero_trust",
];
	...
	it("keeps every retired key out of every catalog", () => {
		for (const [locale, catalog] of catalogs) {
			const resurrected = Object.keys(catalog).filter(
				(k) =>
					REMOVED_LANDING_V2_KEYS.includes(k) ||
					REMOVED_PREFIXES.some((p) => k.startsWith(p)),
			);
