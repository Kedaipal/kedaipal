...
/**
 * Template: header + a single-variant example, a 2-axis multi-variant example
 * (Salmon · Weight × Cut) so sellers see both shapes, and a made-to-order row
 * that fills the two order-rule columns (z8r3fdff97) — the example is how a
 * seller learns the prep unit is MINUTES. Every value is ASCII and comma-free,
 * so the file opens cleanly in Excel with no quoting.
 */
export function buildProductCsvTemplate(): string {
	return [
		VARIANT_IMPORT_HEADER,
		// product_handle,name,description,option1_name,option1_value,option2_name,option2_value,sku,price,stock,weight_grams,prep_minutes,pickup_note
		...
		"puff,Ice cream puff,Baked fresh every morning,,,,,PUFF-1,4.50,40,80,120,Bring an ice bag. They melt in 20 minutes.",
	].join("\n");
}
...
export function buildSampleProductsCsv(): string {
	return [
		...
		"cake,Chocolate cake,Made to order,Size,0.5kg,,,CAKE-05,55.00,0,600,120,Collect from the side counter",
