...
- **Payments:**
  - **Subscription billing (retailers → Kedaipal):** manual v1 live + HitPay Pay-now links, self-serve subscribe and tokenised auto-renewal (`86eyb6z4r`, [`docs/hitpay-recurring.md`](./docs/hitpay-recurring.md)); Stripe deferred (single-vendor HitPay — revisit at 30 paying customers)
...
- **Order stages are configured per FLOW KIND, and the invisible global rename that outranked every kind is retired** ✅ (ClickUp `z8r3fdh3w1`, dev) — the seller's vocabulary was one flat stage list + one global rename map for the whole store, so a kind's own words could be overwritten by another kind's: IndoMart renamed Confirmed → "Ok go" and its **booking** `#ORD-XD7G` read "Ok go"; a renamed `delivered` printed on a finished stay instead of **Checked Out**. `retailers.statusLabels` had had **no write UI since 14 Jun**, so the rename was permanent, invisible and unclearable. **`retailers.orderFlows` is now one stage list per `OrderFlowKind`**, with deliberately **no per-kind rename field** — a stage anchored to `shipped` labelled "Ready to collect" IS the `shipped` rename, so renames and steps are one mechanism and `statusLabels` is migrated into stages, not re-implemented (the ticket's "tag each row with a `flowKind`" sketch was overridden: it leaves `sortOrder` and the ≤20 cap global and gives renames nowhere to live). **Three states per kind**, the middle load-bearing: absent = no answer (LEGACY flat list, delivery/pickup only); `[]` = **"Reset to defaults"**, an explicit answer that OUTRANKS the legacy list, or a reset would silently restore what it cleared; `[…]` = this kind's flow — all in one helper, `configuredStages`. **No backfill needed for the fix:** one line in `resolveStatusLabel` (`FLOW_PRESETS[kind].takesLegacyLabels`) corrects every surface at once; deleting it turns 7 tests red. **Every kind takes custom stages now** — the exemption existed only because one list served the store, and the `≤5 notify stages` cap that argued for restraint no longer exists — so `skippedAnchors` **seeds the defaults, it doesn't forbid a step**, and bulk actions refuse on "no stage in THIS order's resolved list carries the anchor" (`hasAnchor`), not on the preset. One booking config covers stays AND packages. **Two ticket premises were stale:** WhatsApp is no longer a label surface (`86eyd63r8` deleted the sends), and `whatsapp.ts` merely carried `orderStages`/`statusLabels`/`currentStageId` with **no reader** — dead payload, deleted. Settings became one card per kind the store uses, each with its flow as a chain, a Default/Your steps badge, and **Reset to defaults** as a first-class card control (plus Reset all); a customised-but-no-longer-offered kind still shows, labelled, so a config can't go invisible. **A hands-on test then found the reset bug:** the gate was keyed on KIND alone, so "Reset to defaults" cleared the stage LIST and left the RENAME speaking — badge read DEFAULT, toast claimed defaults, chain still read "Ok go" everywhere, and the store could not clear it at all. The gate is now `legacyLabelsApply(orderFlows, kind)`: **an answer for a kind (even an empty one) retires that kind's whole legacy map**, `pending`/`cancelled` included. **Consequence:** the BACKFILL changes those two words for a store that renamed them — the one non-behaviour-preserving step, so the prod `statusLabels` count sizes it before the backfill runs. **Only the widen shipped** — `migrations:backfillOrderFlows` is operator work, and the narrow (dropping `statusLabels`/`orderStages` + their `update` args) is a separate PR, no longer blocked. See [`docs/order-status-customization.md`](./docs/order-status-customization.md#phase-3--stages-are-configured-per-flow-kind-z8r3fdh3w1).
- **IP-based pricing — the region is detected at the edge, the MY/SG pick rides a cookie the server reads** ✅ (dev) — SG-lite shipped both currencies but detection was a stand-in the code named as one: a **time-zone guess**, which is a *device* setting, so an SG visitor whose phone still says `Asia/Kuala_Lumpur` (the common case on this border) was quoted RM79 instead of S$29. Now the visitor's stored pick then `CF-IPCountry`, read **once at the root loader** via **`createIsomorphicFn`** (`src/lib/geo-region.ts`) — no server-fn RPC on any navigation, no server-only import in the client bundle (verified against a real build), and SSR paints the right currency on the first byte with hydration matching. `XX`/`T1` read as "no answer", not as a vote for MY; the time zone survives only where the header is absent (local dev). **The `RegionToggle` stays** on all three surfaces (briefly removed, then restored — geo default + visible override is the near-universal pattern, because geo-IP gets VPNs, carrier NAT and the JB commuter wrong, and without a switcher a wrong guess is a dead end), but **its pick moved from `localStorage` to a cookie** — the server can read a cookie, so an overriding visitor's currency is right in the SSR render instead of flickering after mount. Precedence: **stored pick → geo-IP → time zone → MY**, identical server and client. **`/cost` joined in**, which was the real work: it carried a duplicate of the Pro founding price, an `RM25/hr` labour rate (SG is **S$15/hr — not an FX conversion**, which would be ~S$7 and read as implausible) and a ringgit-shaped AOV slider (SG: max 200, step 2, default 15); `computeStatusQuoCost` now takes the currency as a **required** arg. Two bugs surfaced by rendering the SG page and reading it: the landing anchor said "Starter from RM 79/mo" above S$29 cards, and Scale said "Additional outlets RM49/mo each" beside S$119 — currency had been spelled into the message catalogs, so `pricing-copy.test.ts` now **fails on any currency literal** in `pricing_*`/`pricingpage_*`. Mockup RM amounts stay MY on purpose (a fictional Malaysian seller's storefront, not our price). See [`docs/pricing.md`](./docs/pricing.md).
- **Subscription auto-renewal + Pay-now — HitPay on Kedaipal's own account** ✅ (ClickUp `86eyb6z4r`, dev — sandbox verification pending) — sellers pay Kedaipal with no founder in the loop, on two rails funnelling into the ONE settle path (`settleInvoicePaid`, extracted verbatim from `markPaid` so a webhook-settled founding invoice claims its rank through exactly the manual code): **Pay-now** (every invoice mints a no-expiry HitPay link — billing tab + emails; env creds `HITPAY_BILLING_API_KEY`/`_SALT` only, never a table any seller query can read; absent ⇒ every surface hides and manual billing is byte-identical) and **auto-renewal** (one `save_payment_method` authorisation — MYR card+TnG, SGD card-only, FPX/DuitNow can never appear — then merchant-initiated charges of the INVOICE total, never HitPay's plan cycle, so founding discounts/plan changes stay right and "turn off" is structurally safe: no `autoRenew` on the sub ⇒ nothing can charge). The ticket's premise was half-wrong — the cron never issued renewals (it lock-flipped + logged for Arif), so **renewal auto-issuance was built** and the lapse-lock retired; plus **self-serve subscribe** (`subscribeSelf` — plan+cycle picker, annual = "2 months free" never a %, `foundingIntent` stores auto-get their promised price, founding never otherwise self-servable, admin manual flow fully retained) and the `/pricing` annual toggle flipped live. No charge-failure webhook exists → the sync response is the verdict; no-double-charge = attempt-stamp before the call + `times_charged` reconcile before any retry; dunning 0/+2d/+5d with Pay-now fallback; heal-on-attach; one pre-charge notice per cycle. `/webhook/hitpay` branches V2 JSON events (raw-body HMAC, env salt) vs v1 form (order-first, invoice fallback). Traps: `times_to_be_charged` DEFAULTS TO 1 (set 100 explicitly), 422 on the method list degrades card-only, and every new sub patch leaves `updatedAt` alone (past_due flip-moment invariant, test-pinned). See [`docs/hitpay-recurring.md`](./docs/hitpay-recurring.md).
The full chronological record — what shipped, why it was built that way, what was deliberately rejected, and which trap was found — lives in **[`docs/shipped-log.md`](./docs/shipped-log.md)** (67 entries, newest first). Feature-level reference detail lives in the `docs/*.md` each entry links; [`docs/README.md`](./docs/README.md) is the index.
...
- **A store is operated by its OWNER (`retailers.userId`, strictly one) plus invited TEAM MEMBERS with per-area read/write grants** (86exr91r4, [`docs/team-members.md`](./docs/team-members.md)). Every store-scoped Convex function declares what it needs via `requireRetailerAccess(ctx, retailerId, requirement)` — the requirement argument is REQUIRED (`{area, level}` / `{ownerOnly}` / `{anyMember}`), so forgetting it is a compile error, not a permission hole. Permissions attach to DATA AREAS, never components (a widget declares the area it reads); deny-by-default; billing write + the WhatsApp tab + slug/country/legal-identity/deletion stay owner-only. One store per login; ending a store relationship is always an explicit act, never a side effect (accepting an invite can never cancel a subscription).
