...
subscription is `past_due` **and not comped** — **unless the caller is a Kedaipal
admin** (`isAdmin(ctx)`, the `ADMIN_USER_IDS` allowlist), who is never soft-locked
on any store (their own, dogfooded free forever, or a seller's during act-as). See
[`admin-console.md`](./admin-console.md). Wired (Phase 2) onto seller
