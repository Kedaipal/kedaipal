...
| [fulfilment.md](./fulfilment.md) | Delivery + self-collect as optional, symmetric methods: per-method toggles, the working-method invariant, multi-location pickup library, frozen snapshot lifecycle, the per-product pickup note, WhatsApp confirm composition. |
| [fulfilment-date.md](./fulfilment-date.md) | When the buyer gets it: the date and time (delivery, and pickup when it matters), store + per-product notice, per-product prep time, opening hours incl. split days, seller reschedule. |
