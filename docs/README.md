...
| [seller-mcp.md](./seller-mcp.md) | The seller-facing MCP server (`/mcp`): "ask your business" from Claude/ChatGPT — Clerk OAuth as the tenant boundary, the shared `sellerTools` layer, the 8 read-only tools, free + rate-limited pricing rule, Pro gate + past-due lock. |
