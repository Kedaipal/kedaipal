...
- **Storefront home** — layout is **search → categories → all products** (the "layout A" call, 2026-07-13): the search bar is the first control and **sticky** (always reachable mid-scroll; lives in `ProductGrid`, full-bleed via negative margins), then [`CategoryRail`](../src/components/storefront/category-rail.tsx) **`variant="hero"`** as the page's main highlight — big snap-carousel image cards (bottom scrim, name, description, item-count badge; no image → a deterministic brand-adjacent gradient hashed from the slug) under a "Browse by category" heading — closed by an "All products" divider labelling the full grid below. The rail slots into `ProductGrid`'s **`beforeGrid`** prop and is **hidden while a search query is active** (results take the whole surface). **Search also matches category names** (pass `storeSlug` to `ProductGrid`; shares the rail's `listActivePublic` subscription so it's free) — matches render as tappable "Categories" tiles above the product results, and the placeholder reads "Search products & categories…" when categories exist. The product grid is **denser on desktop** (`lg:grid-cols-5 xl:grid-cols-6`) and hero cards grow at `lg` so a product card never visually outweighs a category card. Sidecar `useQuery`, renders nothing when empty — zero-category stores stay search + grid. (The browser's native search-field "×" is suppressed globally in `styles.css` — every search input ships its own clear button.)
- **Category page** — [`$slug_.c.$categorySlug.tsx`](../src/routes/$slug_.c.$categorySlug.tsx). The `$slug_` underscore is load-bearing: `$slug.tsx` is a leaf with no `<Outlet/>`, so plain dot-nesting would never render. SSR loader mirrors the home page (301 slug-rename redirect **preserving the category suffix**; `notFound()` for unknown/archived/hidden → "Category not found" + a Browse-all CTA, incl. live-archive while open). Own SEO head (`{category} — {store} | Kedaipal`; OG image: category image → cover → logo). Opens with the **same brand header as the home page** (the shared [`StorefrontHeader`](../src/components/storefront/storefront-header.tsx) — cover/logo/name/founding badge/blurb, extracted so the two routes can't drift), then a category block ("← All products" back link + category name + description), then the shared `ProductGrid`/`CartBar` — the cart is localStorage-keyed per retailer, so it carries across pages. **No sibling-category switcher row** (removed 2026-07-13 — buyers hop via back → hero rail; the rail is home-only, hero-only).
