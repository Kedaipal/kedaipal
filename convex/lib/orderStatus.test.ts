...
describe("resolveCurrentStage", () => {
	...
	test("custom stage sharing the current anchor is still honoured", () => {
		const custom = [
			stage({ id: "dispatched", anchor: "shipped", sortOrder: 0, label: { en: "Dispatched" } }),
			stage({ id: "near-you", anchor: "shipped", sortOrder: 1, label: { en: "Near you" } }),
		];
		// Status has caught up to shipped; the seller's specific shipped stage
		// stays selected (equal ordinal, not behind).
		expect(
			resolveCurrentStage({ status: "shipped", currentStageId: "near-you" }, custom)
				?.id,
		).toBe("near-you");
	});
...
describe("stageLabel / stageDescription", () => {
	...
	test("ZH falls back to EN when ZH blank, and is used when present", () => {
		...
		const t = stage({ id: "w", anchor: "packed", sortOrder: 0, label: { en: "Sewing", zh: "缝纫" } });
	...
	test("stageDescription fallback order is [requested, …rest of LOCALES]", () => {
		// ZH unset, EN set → ZH buyer falls back to EN (mirrors the MS behaviour).
		expect(stageDescription(s, "zh")).toBe("Usually 2 days");
		// ZH set → used directly, no fallback needed.
		const withZh = stage({
			id: "v",
			anchor: "packed",
			sortOrder: 0,
		...
		// EN buyer with only a ZH description falls through to it (same "any set
		// locale beats nothing" rule the original EN⇄MS pair had).
...
describe("displayStatusLabel — counter completion", () => {
	test("counter + delivered reads 'Completed', never 'Delivered'", () => {
		expect(
			displayStatusLabel({ status: "delivered", source: "counter" }, "Delivered"),
		).toBe("Completed");
		expect(
	...
	test("storefront / legacy delivered keeps the resolved label", () => {
		expect(
			displayStatusLabel({ status: "delivered", source: "storefront" }, "Delivered"),
		).toBe("Delivered");
...
const CAKE_FLOW: OrderStage[] = [
	stage({ id: "c1", anchor: "confirmed", sortOrder: 0, label: { en: "Order in" } }),
	stage({ id: "c2", anchor: "packed", sortOrder: 1, label: { en: "Baking" } }),
	stage({ id: "c3", anchor: "delivered", sortOrder: 2, label: { en: "Collected" } }),
];
...
describe("configuredStages — the three-state rule", () => {
	test("key absent falls through to the LEGACY flat list, for its kinds only", () => {
		expect(configuredStages(undefined, CAKE_FLOW, "delivery")).toEqual(CAKE_FLOW);
	...
	test("an EMPTY array is 'reset to defaults' and BEATS the legacy fallback", () => {
		...
		expect(configuredStages({ self_collect: [] }, CAKE_FLOW, "self_collect")).toEqual(
			[],
		);
		// …and only for the kind they reset.
		expect(configuredStages({ self_collect: [] }, CAKE_FLOW, "delivery")).toEqual(
			CAKE_FLOW,
		);
	});
	...
	test("a kind's own list wins over the legacy one", () => {
		...
		expect(configuredStages({ delivery: own }, CAKE_FLOW, "delivery")).toEqual(own);
	});
	...
	test("isFlowCustomised reads the same rule", () => {
		...
		expect(isFlowCustomised({ delivery: [] }, CAKE_FLOW, "delivery")).toBe(false);
	});
});
...
describe("one kind's words never reach another", () => {
	const flows = {
		delivery: CAKE_FLOW,
		booking: [
			stage({ id: "b1", anchor: "confirmed", sortOrder: 0, label: { en: "Booked" } }),
			stage({ id: "b2", anchor: "shipped", sortOrder: 1, label: { en: "Arrived" } }),
			stage({ id: "b3", anchor: "delivered", sortOrder: 2, label: { en: "Departed" } }),
		],
	};
...
describe("skippedAnchors seed the defaults — they do not forbid a step", () => {
	test("a booking's DEFAULT pipeline still skips packed", () => {
		...
		expect(hasAnchor(resolveStages({ deliveryMethod: "booking" }), "packed")).toBe(
			false,
		);
	});
...
describe("sameStages", () => {
	test("true when two lists say the same thing to a buyer", () => {
		expect(sameStages(CAKE_FLOW, CAKE_FLOW.map((s) => ({ ...s })))).toBe(true);
