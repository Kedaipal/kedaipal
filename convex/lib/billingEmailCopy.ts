 ...
 * `invoiceOverdue`. `compEnded` (z8r3fdeub2) tells a store an admin turned its
 * comp upgrade off — it's an expired seller now (storefront + buyer ordering
 * live, editing paused until it picks a plan). No bill exists, so it belongs
 * here, not with the invoice emails. Comps have no end date, so there is no
 * "ending soon" notice. */
